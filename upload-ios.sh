...
SCHEME="burnrate-ios"
TEAM_ID="28DMV2MR8T"
ARCHIVE="/tmp/burnrate-ios-upload.xcarchive"
EXPORT_DIR="/tmp/burnrate-ios-export"
EXPORT_OPTS="/tmp/burnrate-ios-export-options.plist"
INFO_PLIST="Sources/iOSApp/Info.plist"
...
echo "==> burnrate iOS v${VERSION} (${BUILD})"
...
if [ -n "${1:-}" ]; then
  ...
  LAST_TAG=$(git describe --tags --match "tf-ios-*" --abbrev=0 2>/dev/null || echo "")
  if [ -n "$LAST_TAG" ]; then
...
if echo "$UPLOAD_OUTPUT" | grep -q "Upload succeeded"; then
  git add "$INFO_PLIST"
  git commit -m "chore: bump iOS build to ${BUILD}"
  git tag "tf-ios-${BUILD}" 2>/dev/null || true
  git push origin main --tags 2>/dev/null || true

  MESSAGE="andrejmat🤖: 📱 burnrate iOS build ${BUILD} (v${VERSION}) pushed to TestFlight

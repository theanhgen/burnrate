...
/// Installs and uninstalls burnrate hooks in ~/.claude/settings.json.
/// Hook commands use the __burnrate_hook function wrapper for identification.
public enum HookInstaller {
    /// The marker function name used to identify burnrate hooks
    static let hookMarker = "__burnrate_hook"
    ...
    __burnrate_hook() { PORT=$(cat "$HOME/.claude/burnrate-hook-port" 2>/dev/null || echo \(HookConstants.defaultPort)); cat | curl -s -X POST "http://localhost:${PORT}/hook" -H 'Content-Type: application/json' -d @- > /dev/null 2>&1 & }; __burnrate_hook
    """
    ...
    public static func install() throws {
        ...
        for event in hookEvents {
            ...
            // Remove any existing burnrate matcher entries for this event
            matcherEntries.removeAll { entry in
                containsburnrateHook(in: entry)
            }
    ...
    /// Uninstalls burnrate hooks from the Claude settings file.
    ...
    /// Detects whether burnrate hooks are currently installed.
    ...
    /// Checks if a matcher entry contains a burnrate hook command.
    private static func containsburnrateHook(in matcherEntry: [String: Any]) -> Bool {

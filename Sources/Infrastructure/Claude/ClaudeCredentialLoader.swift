...
public struct ClaudeCredentialLoader: Sendable {
    ...
    private func loadFromFile() -> ClaudeCredentialResult? {
        ...
        if let bm = bookmarkManager {
            return try? bm.withScopedAccess(folderID: BookmarkFolderID.claude) {
                self._loadFromFile()
            }
        }
        #endif
    ...
    private func _loadFromFile() -> ClaudeCredentialResult? {
    ...
    private func saveToFile(_ data: [String: Any]) {
        #if MAS_BUILD
        if let bm = bookmarkManager {
            nonisolated(unsafe) let capturedData = data
            try? bm.withScopedAccess(folderID: BookmarkFolderID.claude) {
                self._saveToFile(capturedData)
            }
            return
        }
        #endif
    ...
    private func _saveToFile(_ data: [String: Any]) {
        let path = credentialsFilePath
        do {
    ...
    #if MAS_BUILD
    // Native SecItem API for sandboxed MAS builds (Process("/usr/bin/security") is blocked)
    private func loadFromKeychain() -> ClaudeCredentialResult? {
        ...
        guard status == errSecSuccess, let data = result as? Data else {
            if status != errSecItemNotFound {
                AppLog.credentials.error("Failed to load Claude credentials from Keychain: OSStatus \(status)")
            }
            return nil
        }
    ...
    private func saveToKeychain(_ data: [String: Any]) {
        guard let jsonData = try? JSONSerialization.data(withJSONObject: data, options: [.prettyPrinted]),
              ...
              let passwordData = jsonString.data(using: .utf8) else {
            AppLog.credentials.error("Failed to serialize Claude credentials for Keychain")
            return
        }

        // Delete existing item first (ignore errSecItemNotFound)
        ...
        if status == errSecSuccess {
            ...
            AppLog.credentials.error("Failed to save Claude credentials to Keychain: OSStatus \(status)")
        }
    }
    #else
    // Process-based /usr/bin/security for non-MAS builds
    private func loadFromKeychain() -> ClaudeCredentialResult? {
        let process = Process()

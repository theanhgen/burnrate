...
public struct CodexCredentialLoader: Sendable {
    ...
    public func loadCredentials() -> CodexCredentialResult? {
        ...
        if let bm = bookmarkManager {
            return try? bm.withScopedAccess(folderID: BookmarkFolderID.codex) {
    ...
    private func _loadCredentials() -> CodexCredentialResult? {
    ...
    public func saveCredentials(_ result: CodexCredentialResult) {
        #if MAS_BUILD
        if let bm = bookmarkManager {
            try? bm.withScopedAccess(folderID: BookmarkFolderID.codex) {
                self._saveCredentials(result)
            }
            return
        }
        #endif
        _saveCredentials(result)
    }

    private func _saveCredentials(_ result: CodexCredentialResult) {

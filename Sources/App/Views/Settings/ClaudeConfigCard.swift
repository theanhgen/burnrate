...
struct ClaudeConfigCard: View {
    ...
    @State private var claudeProbeMode: ClaudeProbeMode = .api
    ...
    private var configForm: some View {
        VStack(alignment: .leading, spacing: 14) {
            #if MAS_BUILD
            // MAS build: API-only mode, show folder access button
            FolderAccessButton(
                folderID: BookmarkFolderID.claude,
                folderPath: "~/.claude/",
                providerName: "Claude",
            ...
            if !hasCredentials {
                Text("Run `claude` in terminal to authenticate, then grant folder access above.")

...
struct NotificationAlerterTests {
    ...
    private static func stubSettings() -> MockAppSettingsRepository {
        let mock = MockAppSettingsRepository()
        given(mock).proactiveAlertsEnabled().willReturn(false)
        return mock
    }
    ...
    func `shouldAlert returns true for warning status`() {
        let alerter = NotificationAlerter(settingsRepository: Self.stubSettings())
    ...
    func `alert sends notification when status degrades to warning`() async {
        ...
        let alerter = NotificationAlerter(alertSender: mockSender, settingsRepository: Self.stubSettings())

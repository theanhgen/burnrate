...
struct InteractiveRunnerTests {
    ...
    func `run with environmentExclusions strips env vars from subprocess`() throws {
        ...
        let testKey = "BURNRATE_TEST_EXCLUSION_VAR"
        ...
        #expect(!result.output.contains("BURNRATE_TEST_EXCLUSION_VAR=should_be_stripped"))
    }

    @Test
    func `run without environmentExclusions preserves env vars in subprocess`() throws {
        let runner = InteractiveRunner()
        let testKey = "BURNRATE_TEST_PRESERVE_VAR"
        ...
        #expect(result.output.contains("BURNRATE_TEST_PRESERVE_VAR=should_be_present"))
    }
}

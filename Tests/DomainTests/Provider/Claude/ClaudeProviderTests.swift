...
struct ClaudeProviderTests {
    ...
    func `isAvailable returns false in API mode when API unavailable and CLI fallback disabled`() async {
        let settings = makeClaudeSettingsRepository(probeMode: .api, cliFallbackEnabled: false)
    ...
    func `isAvailable returns true in API mode when API unavailable but CLI fallback enabled`() async {
        let settings = makeClaudeSettingsRepository(probeMode: .api, cliFallbackEnabled: true)
        let cliProbe = MockUsageProbe()
        given(cliProbe).isAvailable().willReturn(true)
        let apiProbe = MockUsageProbe()
        given(apiProbe).isAvailable().willReturn(false)
...
private func makeClaudeSettingsRepository(
    probeMode: ClaudeProbeMode = .api,
    cliFallbackEnabled: Bool = true
) -> MockClaudeSettingsRepository {
    let mock = MockClaudeSettingsRepository()
    given(mock).isEnabled(forProvider: .any, defaultValue: .any).willReturn(true)
    given(mock).isEnabled(forProvider: .any).willReturn(true)
    given(mock).setEnabled(.any, forProvider: .any).willReturn()
    given(mock).customCardURL(forProvider: .any).willReturn(nil)
    given(mock).setCustomCardURL(.any, forProvider: .any).willReturn()
    given(mock).claudeProbeMode().willReturn(probeMode)
    given(mock).setClaudeProbeMode(.any).willReturn()
    given(mock).claudeCliFallbackEnabled().willReturn(cliFallbackEnabled)
    given(mock).setClaudeCliFallbackEnabled(.any).willReturn()
    return mock
}
